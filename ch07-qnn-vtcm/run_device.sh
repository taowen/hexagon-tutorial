#!/usr/bin/env bash
#
# run_device.sh — 推送到真机, 运行 QNN VTCM 实验
#
set -euo pipefail
...
DEVICE_DIR="/data/local/tmp/ch07"

# 检查编译产物
for f in "$SCRIPT_DIR/build/aarch64/qnn_vtcm_test" \
...
# 推送测试程序
adb push "$SCRIPT_DIR/build/aarch64/qnn_vtcm_test" "$DEVICE_DIR/"
...
echo ""
echo "=== Running QNN VTCM Experiment ==="
echo ""
