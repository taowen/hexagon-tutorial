/*
 * Chapter 3: QNN Custom Op Test — HVX ReLU → HMX matmul → HVX scale
 *
 * 创建一个 QNN graph 包含 HvxHmxMix 自定义算子, 在 HTP 上执行.
 *
...
int main(int argc, char** argv) {
    printf("========================================\n");
    printf("  Chapter 3: QNN Custom Op (HVX+HMX)\n");
    printf("========================================\n\n");
    ...
    err = g_qnn.backendRegisterOpPackage(backend,
        "./libHvxHmxMix_cpu.so", "HvxHmxMixInterfaceProvider", "CPU");
    if (err != QNN_SUCCESS) { printf("[FAIL] CPU reg=%lu\n", (unsigned long)err); goto done; }
    err = g_qnn.backendRegisterOpPackage(backend,
        "./libHvxHmxMix_htp.so", "HvxHmxMixInterfaceProvider", "HTP");
