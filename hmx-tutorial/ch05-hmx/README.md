...
| 寄存器 | 设置方式 | 作用 |
|--------|---------|------|
| **scale/bias** | `bias = mxmem2(scales_ptr)` | 输出缩放因子，必须在计算前设置 |
| accumulator | `mxclracc.hf` | 清零累加器 |

**`hmx_set_scales()` 是直接 ASM HMX 计算的唯一前置依赖。**

`hexkl_micro_hmx_mm_f16` 内部隐式设置了 scale 寄存器，所以使用 hexkl 的代码
不需要关心。但直接用 `mxmem` 指令时，必须先调 `hmx_set_scales`，否则 HMX 的
scale/bias 寄存器处于未初始化状态，输出是垃圾。
...
**5. `hmx_set_scales` 是唯一的隐式依赖**

直接 ASM HMX 计算只需要一个前置条件：
...
// 设置 HMX scale 寄存器 (每次 matmul 前)
hmx_set_scales(vtcm + scales_off);  // bias = mxmem2(ptr)
```

没有这一步，HMX 的 scale/bias 寄存器未初始化，输出是垃圾。
`hexkl_micro_hmx_mm_f16` 内部自动完成这一步，所以之前从未注意到。
...
| **`hmx_set_scales`** | HMX scale 寄存器初始化 | **Exp 5** | 直接 ASM 必须 |
...
以及**零 hexkl runtime**（Exp 5, hexkl readback → HVX readback 带来 24x 加速）的收益。
